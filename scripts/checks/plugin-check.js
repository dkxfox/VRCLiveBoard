...
for (const id of dirs) {
  ...
  else say('OK', id + ' v' + m.version + ' (api ' + m.api + ', 授权哈希 ' + m.id + '@' + m.version + '|' + m.api + ')');
}
...
if (fs.existsSync(updDir)) {
  for (const f of fs.readdirSync(updDir).filter((x) => x.endsWith('.zip'))) {
    ...
    if (manifests[id].version !== ver) say('WARN', '更新包 ' + f + ' 版本(' + ver + ')落后于源码(' + manifests[id].version + '), 发布前需重建');
...
for (const [id, ent] of Object.entries(def.pluginApprovals || {})) {
  ...
  const want = m ? m.id + '@' + m.version + '|' + m.api : null;
