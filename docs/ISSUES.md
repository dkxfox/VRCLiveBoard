...
- 状态: OPEN(回归: 快捷方式自愈导致图标消失, 已回滚该层, 待用户定位消失位置后重开)
- 回归记录(2026-09-03): 增加"开始菜单快捷方式自愈(带 AUMID+图标)"后用户反馈"图标直接没了"; 已删除已写入的 VRCLiveBoard.lnk 并移除 ensureShortcut 逻辑; 保留首帧修复(show:false + ready-to-show)
- 严重度: S3
