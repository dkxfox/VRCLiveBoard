...
| 子流程 | 何时触发 | 必跑 | 产出 |
| --- | --- | --- | --- |
| **3A 定期检查** | 每次重大改动收尾;或每周 | secret-scan / surface-scan / dep-audit / gate-selftest | 发现项列表(分级 C1-C4) |
...
1. **门禁自测(红队夹具)**:先往临时副本注入 9 类故障(明文密钥 / bat 乱码 / ps1 缺 BOM / 版本只改一处 / 语言包缺键 / 内联脚本语法 / 插件二副本 / 安全开关被关 / 新增未知域名),全部必须被拦住。**这步 FAIL 时,后面所有 PASS 都不算数。**
2. **机密扫描**:工作区(无扩展名白名单)+ **git 历史全部 blob**(密钥进过历史,删文件没用)。
3. **攻击面基线**:出厂代码的外部域名 / 端口 / 危险 API 用法必须与 SECURITY-BASELINE.json 一致 —— 新增任何一项都必须显式进基线。
4. **依赖与产物完整性**:依赖清单变动 + 关键产物哈希(exe/ico/launcher.cs/smtc.py/插件 vendor 聚合)+ npm audit。
5. **常规门禁**:run-gates(G1/G2/GVER/GI18N/GHTML/GPLUG/GCONF)。
6. **隔离冒烟**:临时目录 + 测试端口 19260,8 项端点。
7. **发布包审计**:pack-audit(禁入文件/UTF-8 标志/脱敏/盐一致/恢复备份齐全/机密扫描)+ 生成 SHA256SUMS-v<版本>.txt(把校验和写进 Release 说明公示)。

最后 git-sync-check 确认工作区与 origin 零差。**发布允许条件 = 报告结尾出现 AUDIT PASS,且第 0 步自测 9/9。**

## 3. 基线文件管理

- docs/SECURITY-BASELINE.json 有三个区块:surface(攻击面)、supplyChain(依赖+产物哈希)、updatedAt。
