...
## 199. B站插件: 查实"主播弹幕会推回来" + 发现并修掉**开放平台 CMD 名不同**这个致命坑(2026-09-20, 用户"帮我查一下主播发的弹幕是不是默认不传回API平台")
- 用户问题: 主播自己发的弹幕, 平台是不是默认不推回给我们?
- 查证方法(官方文档是 SPA 抓不到正文, 所以查成熟实现并逐字读代码): **blivedm**(`handlers.py` 的 `_CMD_CALLBACK_DICT` + `models/open_live.py`)与 **blivechat**(`services/chat.py`)。两条都是与官方文档同构、被大量使用的实现。
- 结论 1(**回答用户**): **会推回来**。blivechat 的开放平台弹幕处理里写着 `if message.open_id == client.room_owner_open_id: author_type = 3  # 主播` —— 也就是说主播自己的弹幕默认也会进到我们这边, 靠 **`open_id`**(不是 uid)与 start 返回的 `anchor_info.open_id` 比对来认人。
- 结论 2(**顺手挖出的致命坑, 上播前必须修**): 开放平台的 CMD 名与网页协议**不是一套** —— 弹幕是 `LIVE_OPEN_PLATFORM_DM`、礼物 `_SEND_GIFT`、上舰 `_GUARD`、SC `_SUPER_CHAT`/`_SUPER_CHAT_DEL`(`_DEL` 不是 `_DELETE`)、点赞 `_LIKE`、进房 `_LIVE_ROOM_ENTER`、开播/下播 `_LIVE_START`/`_LIVE_END`、还有 `_DM_MIRROR`(跨房弹幕)与 `_INTERACTION_END`(**服务器主动停推**, 常见于心跳超时 → 必须重新 start)。我们原来的 `kindOf` 只认网页那套 → **这些消息全会落到 UNKNOWN, 上播后一条都显示不出来**。
- 改动: ① `policy.kindOf` 补全 10 个开放平台 CMD + 新类别 `SESSION_END`; ② `events.normalize` 支持开放平台平铺字段(弹幕 `msg`/`open_id`/`fans_medal_*`/`dm_type`(表情弹幕无文字)/`reply_uname`(@前缀)、礼物 `gift_num`/`combo_gift`/`paid`、上舰 `guard_num`/`guard_unit`、SC 金额 `rmb` 与用 `start/end_time` 算时长、点赞 `like_text`/`like_count`), 事件模型加 `openId`/`mirror`/`isAdmin`; ③ `official.normalizeStart` 从 **`data.anchor_info`** 取主播 uid/open_id/房间号/昵称(老写法仍兼容); ④ 插件新增 `ignoreSelf`(**默认忽略主播自己的消息**, 可关)并给**一次性提示**, 免得用户拿自己号测试时以为插件坏了; ⑤ `_INTERACTION_END` → 结束场次并 5 秒后自动重新开局(否则看起来就是"插件莫名其妙不工作了")。
- 证据: 插件离线单测 **215 条全绿**(frame 11 / policy 20 / official 26 / events 59 / session 16 / bridge 50 / index 33); 其中新增的端到端断言: 开放平台弹幕能一路推到聊天框 / 主播自己的消息默认被忽略(且给提示) / 关掉开关后自己也会上聊天框 / 开放平台 SC(`rmb`)认得 / `_INTERACTION_END` 会结束场次并提示重开。文档 `07-接入实现要点.md` 新增"五之二"节, 把这套 CMD 名与字段差异写死, 免得以后再猜。
- 诚实记录: 官方文档页面是 JS 渲染的 SPA(只有 1467 字节外壳), **我抓不到正文**, 所以上面这些是"读成熟实现 + 与官方同构"的证据级别, 不是官方原文; 真连一次就能最终确认(测试连接 + 开播)。任何一条与真环境不符, 我们按真流量回填 `events.test.js` 的夹具。
- 下一步: 用户启用插件、填四个参数、点「测试连接」→ 开播(或先用直播姬)让别人发一条弹幕; 自己发的那条**默认不会上聊天框**, 想验证"主播弹幕也能上"就把设置里的「忽略主播自己发的消息」关掉。
## 200. B站插件 实机联调第一轮: 三个真问题(场次解析 / 结束场次体 / 前端 tr 未就绪)(2026-09-25, 用户实机日志)
- 背景: 用户启用插件、点「测试连接」, 日志里三条 WARN + 一条前端 Promise 拒绝 —— 本轮全部修掉并加了防回归断言。
- 问题 1(**致命, 已修**): `start` 的真实响应是嵌套的 —— `data.game_info.game_id` + `data.websocket_info.{wss_link[], auth_body}` + `data.anchor_info.*`; 而我按"扁平 key"解析 → 场次号/弹幕地址/auth_body **全是空**。连锁后果: ① 连接直接失败("开放平台没有返回弹幕服务器地址或 auth_body"); ② 结束场次发出空 game_id 被平台拒("缺少 game_id"); ③ **每次 start 都在平台侧留下一个没关掉的场次** → 很快撞 **7010「插件或直播工具同一房间启动数量超过配置上限」**。修法: `normalizeStart` 认真实形状(扁平写法仍兼容), `startSession` 拿不到场次信息时抛人话错误并带上 **data 的键名**(只出键名 —— 值里可能有 auth_body 这类令牌, 绝不能进日志), `endSession` 没有场次号时**跳过**而不是照发。
- 问题 2(已修): `end` 的请求体是 `{app_id, game_id}` —— 我原来复用了心跳体(只有 game_id); 另外 **7000/7003「项目已关闭」按成功处理**(与 blivedm 同口径), 免得下播后刷一片"结束场次失败"。
- 问题 3(已修): 前端 `[WARN] Promise拒绝: tr is not defined` —— `tr` 定义在 **app-security.js**, 而它在 app.js **之后**才执行, 于是加载期(微任务或"立刻返回"的回调)调 tr 就会踩空(与 GBOOT 那条"app.js 里不许同步用 tr"同源, 这次是异步路径)。修法: app.js 顶部放一个 `window.tr` 兜底(先按原键显示 + console 提醒一次), app-security.js 执行时用真 tr 覆盖掉它。
- 证据: 插件离线单测 **222 条全绿**(official 26→33: 真实响应形状解析 / end 体带 app_id / 心跳体不带 app_id / 无场次号跳过 end / 7003 视作成功 / start 空 data 的人话错误); **契约测试的假 start 响应也改成了真实形状**。
- 诚实记录(本轮最重要的教训): 根因是**我最初对 start 响应的假设就是错的**, 而且我的离线夹具是照着这个错假设写的 —— 所以"222 条全绿"完全掩盖了它, 真正抓住它的是用户的实机日志。**夹具必须对着真实报文写**(或至少标注"这是臆想形状, 待真机核对"), 否则离线绿灯只是自我确认。用户房间现在可能还压着几个未释放的场次(7010), 得等平台按心跳超时释放(几分钟)或重启直播。
- 下一步: 等几分钟后重试「测试连接」(成功后会自动开始接收) → 开播/直播姬验证头顶弹幕。
## 201. B站插件: 修"弹幕带昵称关了还带昵称" —— 桥快照了配置, 没有热更新(2026-09-25, 用户实机反馈)
- 现象(用户原话): 「弹幕带昵称这个功能没用, 关了还带昵称」。
- 根因: `createBridge` 在**创建时把配置快照**进自己的 `cfg`(policy 的 `cfgOf` 是 `Object.assign({}, DEFAULT, cfg)` 的拷贝), 而插件只在 `start()` 里 `makeBridge()` 一次。设置面板保存只改了 `ctx.config`(内存里那个对象确实变了), **运行中的桥却一直拿着旧配置** —— 于是"设置看起来没用", 而且保存回执还写着"已保存 ✓"(没提重启), 更让人以为是坏功能。同一个坑对前缀/限流/显示时长/屏蔽词/卡片优先级**全都成立**。
- 改动: ① 插件把桥配置抽成 `bridgeCfg()` 并加 `syncCfg()`; **每秒 tick 先同步再走桥**(1 秒内收敛, 也覆盖"直接改 config.json"的情况); ② 新增 `api.reloadConfig`(返回当前真正生效的配置), 前端**保存成功后立刻调用**它 —— 即改即生效, 不用重启插件; ③ `status.effective` 报出真正生效的 showUname/prefix/throttle/优先级, 以后排查"设置没生效"直接看它。
- 证据: 契约测试新增 5 条(默认带昵称 → 关掉后**不重启**第二条就不带 → 改回去靠每秒 tick 也能生效 → `reloadConfig` 回报生效值 → `status.effective`), 插件离线单测 **227 条全绿**; 门禁 frontend-boot 新增断言: 保存后没调 `reloadConfig` 就 FAIL(把这次的回归钉住)。
- 诚实记录: 这是**"设置面板做得像模像样、底层却不生效"**的典型 —— 上一轮我把热更新能力(`bridge.setCfg`)写好了却没接线(典型的"写了一半")。用户一句"没用"比任何离线断言都有效: 我的 222 条断言全绿, 因为它们都在**改配置之后重新建桥**, 从没测过"运行中改配置"。
- 注意: 用户当前跑的是旧代码,**这一次仍需禁用/启用一次插件**(或重启程序)才会加载新代码; 之后改设置就即时生效。
## 202. B站插件: 修"弹幕排队 30 秒后被丢弃" —— 会话重开重建桥 + 测试连接自己开局次(2026-09-25, 用户实机日志)
- 现象(用户原话): 「查一下中间这些为什么丢弃了」—— 日志里一串 `丢弃(expired): DKXfox: 带昵称测试`, 但聊天框里确实显示了几条。
- 原因 1(**主因**): 每次"平台主动停止推送 → 5 秒后重开"时, 我在 `start()` 里**重建了整个桥**(`bridge = makeBridge()`)。新桥丢了"自己刚推的那条文本"的记忆 → 还挂在屏上的**我们自己的**上一条 transient 被当成"别人的同优先级占屏"(`decideDisplay` → `lower-priority`)→ 后面的弹幕一直排队等它让位 → 30 秒时效到 → 整批 `expired` 丢弃。实测日志里每次重开(13:46:43 / 13:47:45 / 13:49:06)之后都跟着一条丢弃, 时间线完全吻合。
  → 修: `if (!bridge) bridge = makeBridge();` —— **桥跟着插件走, 不跟着会话走**(配置变化由 `syncCfg()` 负责); 记忆与队列都不再丢。
- 原因 2(**自己制造的**): 13:46:38 与 13:49:01 那两次"平台主动停止推送"其实是**我自己点「测试连接」造成的** —— 插件已经在场次里接收, 测试又 start 了一个新场次, 平台把原来那个掐掉 → 我们收到 `INTERACTION_END` → 重开……循环(顺带解释了之前撞的 7010)。
  → 修: `api.test` 在 `status.running` 时直接返回 `{ok:true, already:true}`, **不再新开场次**。
- 顺带两处: ① 过期/丢弃日志现在写明**等不到上屏的原因**(如 `respect-transient`、`protect-source:livetranslate`、`throttled`), 插件侧用 WARN 打出来 —— 用户再看到丢弃就知道被谁挡着; ② `bridgeCfg()` 现在把策略层可调项(`danmakuQueueTtlMs`/`queueMax`/`highValueQueueMax`/`respectPriority`/`preemptBackground`/`protectSources`/`aggregateWindowMs`/`aggregateFormat`/`aggregateKinds`)从 config.json 透传到桥(以前设了不生效)。
- 证据: 插件离线单测 **233 条全绿**(index 38→44: 平台停推后自动重开且**弹幕照常上屏**、被 99 优先级占屏时丢弃日志带 `respect-transient`、运行中「测试连接」不新开场次、策略项透传); 门禁 + 隔离冒烟通过。
- 诚实记录: 这是**第二个**"我自己制造问题再排查"的案例(M-20260920 那次是占位导出形状、这次是测试连接掐自己场次): 以后凡是"可能在运行中又被触发一次"的操作(测试/预览/重开), 先问一句"它会不会动手改我正在用的东西"。
- 设计取舍(写给用户): 高价值(SC/礼物/上舰)**永不超时丢弃**, 普通弹幕 30 秒时效是刻意的(陈年弹幕补发没意义), 想改可调 `danmakuQueueTtlMs`。
## 203. B站插件: 修"弹幕一直被挡住"的死锁 + 卡死保护 + 排队日志说清是谁占屏(2026-09-25, 用户"压缩的弹幕不会推送")
- 现象: 上了条目 202 的修复后, 丢弃日志终于带上原因 —— 全是 `等不到上屏: lower-priority`; 用户补充"压缩的弹幕不会推送"(聚合出来的 `666×2` 也在丢弃列表里)。
- 判断: **不是聚合坏了** —— `666×2` 已经生成并进了队列, 只是**整条链路都被同一个占屏者挡住**, 30 秒时效到就一条条过期。`lower-priority` 的含义是"当前屏上那位优先级 ≥ 我们(75), 按你定的规矩让路"。
- 根因(**我方策略里的死锁**): `src/composer.js` 的 `composer.current` **只在真的发出去时更新, 而且过期也不清空**。于是上一屏那条临时文本(插件欢迎语/公告之类)明明早就过期了, 仍然被我们当成"占屏者"; 我们再为它让路 → **我们等它让位, 它永远不让**。条目 202 修掉"重开丢记忆"之后, 这个更深的问题就暴露出来了。
- 改动: ① `policy.decideDisplay` 新增 **`stale-transient`**: `cur.ttlUntil` 已过 → 当作空场直接显示(根治死锁); 数据源没有 ttlUntil 字段, 不受影响。② bridge 新增**卡死保护** `stuckEscapeMs`(默认 20 秒, 0=关): 被同一个**非保护来源**挡住超过这么久就抢一次并记 `stuck-escape:<原原因>`; **来源保护(截图翻译/翻译字幕)与 `preemptBackground=false` 永远不越过**。③ 排队时打**限频 10 秒**的日志说清占屏者: `排队中(等不到上屏: lower-priority, DANMAKU) 当前占屏: pages/90 "公告…" (有效期到 14:01:30)`; `status.screen` 也一并报出。
- 证据: 插件离线单测 **242 条全绿**(policy 20→23: 残影/未过期/无 ttl 三种; bridge 50→56: 5 秒内老实排队、超时抢一次、来源保护永不抢、`stuckEscapeMs=0` 全关、残影直接显示)。
- 诚实记录: 这个死锁**本来就在策略层里**(条目 192 写 `decideDisplay` 时就埋下了), 之前被"重开丢记忆"掩盖着 —— 说明当时对 `composer.current` 生命周期("过期不清空")的理解只写进了注释, 没有变成断言。现在它变成三条断言了。
- 下一步(用户): 重启插件拿新代码; 若仍有丢弃, 日志会直接写明**是谁在占屏**, 再决定是调优先级还是把它加进卡死保护的例外。
## 204. B站插件: 新增"高价值保持展示(SC 持久化)"开关 + SC/礼物/上舰逐类时长(2026-09-25, 用户要求)
- 需求(用户原话): 「加一个功能开关, SC留言持久化, 可以设置持续时间, 其它礼物也可以设置持久时间。这段时间内保持展示。」
- 先说清原有的**真问题**(用户日志里能看到): 14:18:30 推了 SC(优先级 88), 14:18:40 一条弹幕就把屏幕换掉了 —— 因为桥的 `ownOnScreen()` 只比"文本是不是我最后推的", **不管类别**, 所以"自己刚推的那条"永远不算占屏, 高价值消息会被下一条弹幕按节流窗口(1.5 秒)顶掉。
- 改动: ① bridge 新增 `holdEnabled`(开关)与 `scHoldMs`/`giftHoldMs`/`guardHoldMs`(逐类时长, 0=用基础时长 `highValueTtlMs`); ② `doPush` 记住 `holdUntil`/`holdKind`, `ownOnScreen(now)` 在**保持窗口内把自己也算成占屏** → 弹幕自然排队(原因 `respect-transient`, 日志可读)、窗口结束自动恢复"自己不算挡自己"; ③ 开关关掉 = **不守屏**(下一条消息过了节流就能替换, 即老行为) —— 这样开关才有真作用; ④ `status.queue` 报出 `holdKind`/`holdUntil`, `status.effective` 报出四个保持参数; ⑤ 插件设置里新增 4 个字段(开关 + 三个时长), 保存即生效(热更新)。
- 证据: 插件离线单测 **256 条全绿**(bridge 56→67: SC 按 60 秒保持、期间弹幕排队且原因可见、窗口内 tick 不换屏、**保持比弹幕时效长时排队弹幕会过期丢弃**(如实断言, 并提示要同时调 `danmakuQueueTtlMs`)、短保持窗口结束后正常补发、开关关掉回到老行为、`scHoldMs=0` 用基础时长、礼物/上舰各自生效; index 44→47: 端到端 SC 60 秒 + status 可见 + 时长可热改)。
- 诚实记录: 这条需求其实是**用户先看到现象("弹幕把 SC 顶掉了")再提的功能**, 说明我原来把"自己的消息不算占屏"这条规则写得太绝对 —— 它对普通弹幕是对的(否则自己饿死自己), 对高价值消息恰恰错(它就该占着屏)。规则要按类别分, 不能一刀切。
- 下一步(用户): 重启插件 → 设置里能看到「高价值消息保持展示」开关与三个时长; 想让 SC 挂 60 秒就填 60000(建议同时把 `danmakuQueueTtlMs` 调大, 否则期间弹幕会过期)。
## 205. B站插件: 固化设置契约 + 直播间信息展示(标题/房间号/热度开关)(2026-09-25, 用户"固化刚才的内容, 增加直播间信息展示开关")
- 用户要求: ① 固化刚才那些改动; ② 增加直播间信息(标题/房间号/观看热度)的展示开关; ③ 让我提还需要加什么。
- 固化 1(**设置契约测试**, 新 `test/settings.test.js`, 19 条): manifest 里声明给控制台渲染的每个字段, **必须**是代码真的会读的键(键集 = 插件 DEFAULTS ∪ 桥/策略默认表 ∪ 四个凭据 ∪ 透传表); 另校验 type/label/默认值类型/域名声明/用户点名的 10 个开关都在。起因就是前两轮连续踩"设置面板像模像样、底层不生效"(弹幕带昵称、保持展示), 现在这条契约变成断言, 摆设开关会被门禁抓住。
- 固化 2: 把"还没做成界面字段、只能改 config.json"的 13 个键(`danmakuQueueTtlMs`/`queueMax`/`highValueQueueMax`/`respectPriority`/`preemptBackground`/`protectSources`/`aggregate*`/`blockedWords`/`kinds`/`timeoutMs`/`restartDelayMs`/`stuckEscapeMs`)在测试输出里**如实列出来** —— 免得以后误以为界面上都能改。
- 直播间信息(新): `lib/roominfo.js`(纯函数: 人数格式化 万/亿、文案拼装、公开接口返回的宽容读取)+ 插件里注册一个**低优先级数据源**(`id: roominfo`, 默认优先级 8 / 60 秒刷新, 跟电脑状态、公告板同一套机制 → 只在没别的可显示时才轮到它, 不抢弹幕的屏)。数据来源**诚实区分**: 房间号来自官方 start 的 `anchor_info`(一定有); 标题来自官方开播事件 `LIVE_OPEN_PLATFORM_LIVE_START.title`; **热度官方通道不提供** —— 所以"显示观看热度"是**独立开关且默认关**, 打开后才去请求公开网页接口 `api.live.bilibili.com/room/v1/Room/get_info`(只读、免登录), 顺手也用它的标题纠正开播那一刻的旧标题。manifest 相应新增该域名(授权弹窗里用户看得到)。
...
## 207. 流程检查(PROCESS-02)+ 插件完好性测试: 补功能卡、补门禁、抓两个真问题(2026-09-25, 用户"进行PROCESS-02流程检查, 对插件完好性测试")
- 范围: `plugins/bilibili-live` 的全部改动(DEV-NOTES 188~207)对照 PROCESS-02 逐条检查; 结论表写进 `PROCESS-02 §8.10`, 这里只记要点。
- **补上的流程缺口**: ① **功能卡**(D0)原来没有 —— 已回补 `docs/FEATURES/F-20260925-01-B站直播插件.md`(七问 + 四契约 + 切片 + 7 条 ASSERT + 发布准备清单), 跑 `feature-accept.ps1` → **19 PASS / 0 FAIL**; ② **优先级阶梯**(§2)原来没有 B站插件的位置 —— 已补 4 档(上舰 92 / SC 88 / 礼物 80 / 弹幕 75 / 直播间信息 12), 并标注"礼物 80 与临时推送 80 同值、同值先到先得"; ③ **插件改版五件事**(§3): 版本号从 0.2.0 一路没动(期间改了 10 轮) → 已升 **0.3.0** + 新增插件 `CHANGELOG.md`(写明"**需重新红窗授权一次**"); ④ 使用说明/版本说明**待发版时补**(插件还没随包发布), `dist/插件更新包` 同理。
- **为"插件完好性"新增的测试能力**(以后每个插件都受益): ① **GPLUG 门禁连带跑插件离线单测**(以前 300+ 条断言从不进门禁) —— 上线第一次就抓到一条过期断言(优先级 8→12); ② **隔离实例完好性用例**(backend-flow ⑦c): 批准 → 启用 → 调接口(status/preview/test)→ 停用 → 再启用, 并断言"没凭据不联网""直播间信息数据源已注册""停用后接口不可调"; ③ **隔离实例的 OSC 出口改到 19259**(不再是用户 VRChat 的 9000), 从此任何 fixture 都能安全地推聊天框文本(这条纪律以前只能靠"没人写过推送用例"维持)。
- **新用例上线即抓到两个真问题**: ① **插件注册的数据源缺 `enabled`** → composer 永远不轮询 → "房间号/标题不显示"(已修: 框架侧默认启用 + 插件侧显式写, 并写进 `PLUGIN-DEV.md`); ② 我的夹具断言写死了"缺四个参数", 但前一个用例刚写过假 app_id/secret → 改成"至少缺 access_key_id"(夹具要吃自己的副作用)。
- 证据: `run-gates.ps1 -Smoke` → **15 PASS / 0 FAIL**; `feature-accept.ps1`(功能卡)→ **19/0**; 插件离线单测 **305 条**全绿(已经由 GPLUG 把关); 隔离冒烟 `backend-flow` **128 PASS / 0 FAIL**(其中插件完好性 14 条)。
- 诚实记录: **流程检查的价值就在"发现自己的流程没走完"** —— 功能卡、版本号、优先级阶梯这三项都是"代码写完了但流程没走", 用户不查就一直是暗账。另外 `smoke.ps1` 有 BOM, 我用 edit 改完 BOM 被吃掉, 按老办法字节级补回(这条教训 DEV-NOTES 里已经出现过两次, 这次是第三次 —— 已写进本节, 下次改 ps1 一律先验首三字节)。
- 用户侧待办: 插件版本升到 **0.3.0** ⇒ 授权哈希失效 ⇒ 在控制台**重新授权一次**(然后启用); 之后按功能卡的"用户确认"项实机验证(填四参数 → 测试连接 → 开播看弹幕与直播间信息)。
## 208. PROCESS-03 安全审计(3A 定期检查)+ 新插件加固: 4 个发现项全部处置(2026-09-25, 用户"走一遍PROCESS-03确认无安全漏洞")
- 范围: 3A 定期检查(secret-scan / surface-scan / dep-audit / doc-consistency / auth-state-check)+ 对**本轮新增攻击面**(B站插件)做代码级审计。3B/3C(发布前检查与产物哈希)本次不发版, 按流程留到发版时跑。
- **通过项**: secret-scan(工作区 268 文件 + git 历史 971 blob)未发现机密特征; doc-consistency 0 FAIL; auth-state-check 0 FAIL/0 WARN; dep-audit 的依赖清单与 8 项受监控产物哈希全匹配。
- 发现 1(**C3, 审计器自身假通过, 已修**): `dep-audit` 打印 `OK npm audit: 0/0/0/0`, 而 npm 其实因 `ENOLOCK`(UNC 工作区 + 仓库根没有 lockfile)**根本没跑**; 旧 catch 分支把错误 JSON 里不存在的 `metadata.vulnerabilities` 当成 `{}` → "零漏洞"。改成"没有 metadata 就报 WARN + 原因, 不算通过"。→ ISSUES M-20260925-01。**这正是 PROCESS-03 §0 说的"扫描器有盲区却没人发现"**, 而且是**看起来全绿**的那种。
- 发现 2(**C2, 新插件, 已修**): 弹幕服务器地址来自 start 响应(**外部输入**), 而认证帧里带 `auth_body`(令牌) —— 原实现照单全收, 篡改后的响应能把令牌发到任意主机。→ `session.js` 加 `hostAllowed()` + `allowHosts`(默认 `*.chat.bilibili.com` + 回环), 白名单外不建连接; 9 条新断言(含"伪造后缀 `chat.bilibili.com.evil.com` 必须拒")。→ ISSUES M-20260925-02。
- 发现 3(**C3, 新插件, 已修**): 帧解析缺三道闸 —— 对端可声称 64MB 帧、可用 brotli 把几百 KB 解成几百 MB(压缩炸弹)、可无限嵌套压缩、接收缓冲无上限。→ `frame.js` 三道闸(单帧 8MB / 解压后 4MB / 嵌套 ≤3 层)+ `session.js` 8MB 缓冲上限; 4 条新断言(并验证正常压缩帧没被误伤)。→ ISSUES M-20260925-03。
- 发现 4(**C4, 扫描器盲区, 已修**): `surface-scan` 注释写着"只扫会随包出厂的代码", 实现却不知道 `scripts/pack-exclude.json`, 于是把 `plugins/bilibili-live/test` 里**永不进包**的夹具算进攻击面(报出假域名 `i0.hdslb.com`/`x` 与测试脚本里的 child_process)。→ 改为读 pack-exclude; 复跑后只剩两个**真实**新增域名。→ ISSUES M-20260925-04。
- **基线变更(PROCESS-03 §3 要求写明)**: `docs/SECURITY-BASELINE.json` surface.domains **14 → 16**, 新增 `live-open.biliapi.com`(官方开放平台接口, 插件核心)与 `api.live.bilibili.com`(**仅**"显示观看热度"打开时用的公开只读房间信息接口, 默认关); 端口/危险 API 类别无变化; `updatedAt` → 2026-09-25。两项都已在插件 manifest 的 permissions 里显式声明(用户在授权红窗看得到)。**谁审过: 本次审计逐项复核**(扫描器修正后重跑, 只多出这两个, 没有别的漂移)。
- 证据: 插件离线单测 **319 条全绿**(frame 17 / policy 23 / official 33 / events 59 / session 25 / bridge 67 / roominfo 17 / settings 19 / index 59); surface-scan 复跑全 OK; dep-audit 复跑如实 WARN; 门禁 15 PASS(含隔离冒烟)。
- 残余风险与边界(**不敢说"零漏洞"**): ① 插件权限是**契约式**的 —— 插件若直接 `require('fs')` 仍可绕过, 真正的信任边界是红窗授权(PROCESS-02 §2 已写明); ② 框架不拦插件自己发起的 WebSocket, 本次是插件自己加的白名单, 框架级强制需另立卡; ③ "显示热度"用的是**非官方、未文档化**的公开网页接口(只读公开数据、免登录、默认关), 合规上要用户知情; ④ 日志里保留 access_key_id 前 4 位(设计如此, 见 `07` 第六节); ⑤ `npm audit` 目前无法给出结果(依赖侧仍靠产物哈希 + 清单比对部分覆盖), 是否加根 lockfile 待用户决定; ⑥ 3B/3C 未执行 —— 发版前必须跑, 且插件进包会让 `zipVolumes` 基线漂移(发版时重打包并更新)。
## 209. B站插件发布: bilibili-live@0.3.0 进市场(official 分级)+ 修市场脚本两处坑(2026-09-25, 用户"发布这个插件, 下次更新新版本再跟随进包")
- 用户决定: **插件先单独发**(市场渠道), 随包出厂放到**下一个程序版本**一起走。
- 发布产物: `market/packages/bilibili-live-0.3.0.zip`(**39KB**, sha256 `e54fb7e1d589…`) + `market/index.json` 新增一条(`tier: official`、`minApp: 1.4.4`、带 tags 与 changelog); 同时按 PROCESS-02 §3.4 生成 `dist/插件更新包/bilibili-live-0.3.0.zip`(GPLUG 比对版本一致)。用户侧路径: 控制台「插件市场」→ 刷新目录 → 安装(仍需过一次红窗授权)。
- **先修了市场脚本的两处坑**(否则这次发布会出事): ① `--only` 原来会把没点名的插件**从 index.json 里删掉**(等于"新增一个插件就把其余全部下线")→ 改成与已有目录**合并**(点名的覆盖、其余按原顺序保留); ② 打包原来只排 `data/` 与 `node_modules/`, **不知道 pack-exclude** → `plugins/bilibili-live/test/`(300+ 断言 + 假服务器 + child_process)**会被发给用户** → 改成读 `scripts/pack-exclude.json` 排除对应条目。证据: 复跑后 index 仍是 5 条(其余 4 条 tier/size 不变), `tar -tf` 列表只有 `manifest.json / index.js / lib/* / README.md / CHANGELOG.md`, 没有 test/。
- 插件 manifest 补市场字段: `tags`、`minApp: 1.4.4`、`changelog`(市场卡片直接展示"0.3.0 新增什么 + 需重新授权")。
- **诚实提醒(兼容性)**: 本插件依赖"控制台按 manifest.settings 自动渲染设置字段", 而这能力**是这轮才加的、还没随程序发布**; 1.4.3 用户即使从市场装上, **插件卡片里不会出现设置输入框**(只能手改 config.json)。条目里写了 `minApp: 1.4.4`, 但客户端**目前只展示、不强制** —— 建议下一个程序版本同时带上"设置字段渲染器 + minApp 强制", 那时这个包才对普通用户真正可用。
- 证据: GPLUG **17 PASS / 0 FAIL**(含 `OK 更新包 bilibili-live-0.3.0.zip 与源码版本一致` + 插件离线单测 319 条); `run-gates.ps1 -Smoke` 15 PASS。
- 下一步(发 1.4.4 时): ① 插件随包出厂(make-dist 会自动生成 `官方可选插件` 备份, 打包清单无需改); ② 是否把本插件写进 `config.default.json` 的 `pluginApprovals`(**那会让新装用户免红窗**, 要用户明确同意才做); ③ 客户端补 minApp 强制; ④ 打包后按 PROCESS-03 3B 跑 release-audit(插件进包会让 `zipVolumes` 体积基线漂移, 需重打包并更新)。
## 210. 本轮"主程序侧"的坑总账 + 待修补清单(2026-09-25, 用户"总结一下这轮在主程序上发现的坑")
- 口径: 只算**主程序**(src/ + scripts/checks + 控制台 + 流程/文档), 插件自身的坑(场次解析、弹幕被挡等 8 处)另见条目 200~203。
- **已修(10 项)**: ① 插件注册的数据源缺 `enabled` 默认值(`manager.registerSource`)→ 注册即启用 + 文档补; ② 缺 `ctx.chatbox.current()`(桥看不到占屏者)→ 新增只读能力(返回副本); ③ 插件设置"能写回 config"落地: `manifest.settings` → 控制台自动渲染 + 密钥不回传 + 保存后热重载 + GBOOT 断言; ④ `tr is not defined` 兜底; ⑤ dep-audit 的 npm audit **假通过**(ENOLOCK 被当 0 漏洞)→ 报 WARN; ⑥ surface-scan 不知道 pack-exclude → 把不进包的测试文件算进攻击面; ⑦ 隔离实例会把 OSC 打到用户 VRChat 9000 → 改 19259; ⑧ 缺"插件生命周期"完好性用例 → backend-flow ⑦c; ⑨ GPLUG 不跑插件单测 → 连带跑(319 条); ⑩ `make-market.js` 两处(--only 抹目录 / 打包不含 pack-exclude)。
- **文档/流程已同步(4 项)**: 优先级阶梯表补 B站插件 4 档; 补功能卡 F-20260925-01(含 7 条 ASSERT); 插件 `CHANGELOG.md` + 版本 0.3.0; `README.md` 的 Node 版本口径从 "18+(建议 20+)" 改成 **22+**(engines 已是 >=22, 之前文档与实现不一致)。
- **待修补(按建议顺序)**:
  1. **M-20260925-05 `tr` 的根因**: 把 i18n 取词/词典初始化从 app-security.js 挪到 lang.js(或 i18n.js, 排在 app.js 前), GBOOT 加"app.js 执行前 tr 已可用"断言。**为什么排第一**: 兜底只是不让它炸, 文案仍可能显示成 key, 且这个"依赖后加载脚本"的模式还会再犯。
  2. **M-20260925-06 生命周期用例泛化**: 让 `plugins/` 下每个插件都跑"启用→调接口→停用"和"数据源 enabled"断言(本轮那种"注册了却永不轮询"的坑, 泛化后能在每个插件上自动抓到)。
  3. **M-20260920-01 / M-20260925-08 插件面板**: 先给结论(补齐 or 废弃摘按钮), 再决定 4 个官方插件设置是否迁到 `manifest.settings`。现在"用户看得见点了没反应"是明确的体验债。
  4. **M-20260925-01 的根因(依赖审计跑不起来)**: 仓库根没有 `package-lock.json` → `npm audit` 永远 ENOLOCK。加 lockfile 属 A0 决策(**待用户点头**), 加完依赖侧才有真正的漏洞扫描。
  5. **M-20260925-07 `feature-accept.ps1` 的引号坑**: 改传参方式, 别让写功能卡的人踩。
...
## 212. 待修补 3~5 落地: 面板死按键摘除 / 依赖审计真跑起来 / 断言引号坑(2026-09-25, 用户"继续依次完成修补, 准备发144")
- **③ 插件面板给了结论**(M-20260920-01 → WONTFIX-面板 / FIXED-死按键): 决定**不支持插件自带 HTML 面板**(容器缺失已久、没有真实需求方); 摘掉「打开页面」死按键与关闭残留逻辑, 带 `panel` 的插件改为显示一行三语说明(`plgPanelGone`, 仅当该插件**没有** `settingsUi` 时), 设置统一走 `manifest.settings`; `PLUGIN-DEV.md` 新增一节写明"本版不支持面板 + 想恢复要另立功能卡(容器+渲染壳+方法桥+沙箱 iframe)"。M-20260925-08(官方插件设置迁到 manifest.settings)保持 OPEN, 作为后续独立事项。
- **④ 依赖审计从"假通过"变成"真的在跑"**(M-20260925-01 收尾): ① 生成并提交根 `package-lock.json`(26KB; 在本地临时目录生成后拷回 —— UNC 工作区里 npm 的 cmd 包装器跑不了); ② 修 `dep-audit` 执行 npm 的方式: Windows 上 `.cmd` 必须 shell 才能 spawn, 而 shell(cmd.exe) 不接受 UNC 做 cwd(会静默退回 C:\Windows, 于是在**别的目录**里找 lockfile → 永远 ENOLOCK)—— 改为**用当前 node 直接跑 npm 的 CLI js**。复跑: `OK npm audit: 严重 0 / 高危 0 / 中 0 / 低 0(合计 0)`。
...
## 215. 开发数据记录: 1.4.4 这一轮(2026-09-25, 用户"开发数据记录")
口径: 从插件立项那次提交 `de7a638`(2026-09-20, 条目 188)算到发布后的 HEAD; 数据来源 = git / 插件测试运行器 / 发布审计报告 / 打包输出, 都能复现。

| 项 | 数值 |
| --- | --- |
| 提交数 | **38** 个(2026-09-20 → 2026-09-25) |
| 代码改动规模 | **65 个文件, +5313 / −788 行** |
| 插件代码 | lib **963** 行 + index **315** 行 = 1278 行(零第三方依赖) |
| 插件测试 | **1227** 行 / **9** 个文件 / **319** 条断言(全离线, 不需要凭据) |
| 门禁新增能力 | GPLUG 连带跑插件单测; plugin-behavior 通用生命周期 5 插件 **45** 条; backend-flow 插件完好性 **14** 条 |
| 隔离冒烟 | backend-flow **134** PASS / 0 FAIL(含市场 minApp、插件生命周期、密钥掩码) |
| 发布审计 | 十步全 PASS(**AUDIT PASS**), 报告 `审计报告-AUDIT-20260925-173918.txt` |
| 交付物 | Desktop **215.59 MB**(sha256 090fccaa…) + Lite **8.06 MB**(e8042a67…) + SHA256SUMS |
| 插件市场 | 目录 **5** 条, 其中 bilibili-live **0.3.0**(official, minApp 1.4.4, 39KB) |
| 记录 | DEV-NOTES 条目 **188–214**(27 条) + 条目 215 本条; ISSUES 卡片 **10** 张(2 张 09-20 + 8 张 09-25) |
| 清理 | 删掉 `dist/_旧编号测试包-1.4.1`(**700 MB** 旧测试包) 与 `config.json.bak`(含密钥的旧备份, 可再生) |

**这一轮最值得记的三个数**: ① 插件 319 条断言里有 **13 条**是实机踩坑后补的(白名单 9 + 帧三道闸 4), 另有 3 类问题(设置不生效 / 数据源没 enabled / 弹幕被占屏挡住)是**实机才暴露**的; ② 门禁侧新增 **59 条**能力断言(45 + 14), 目标就是让"实机才发现"的比例下降; ③ 主程序侧的坑 **14 项**, 其中 5 项是**我自己引入又被自己的检查抓到**(BOM 三次、假通过、扫描面、OSC 出口、--only 抹目录)。
## 216. 今日收尾(2026-09-25): 1.4.4 发布 + 清理 + 文档 + VR 虚拟键盘立项研究
- **发布**: 1.4.4 打包 → 发布前审计(十步 **AUDIT PASS**) → 上线(3 资产, GitHub digest 与本地 SHA256SUMS 逐份一致, latest = v1.4.4); 途中修掉"恢复备份不遵守 pack-exclude → 插件 test/ 进包"这个真问题并更新体积基线。
- **主程序侧坑收尾**: 14 项里已修 10 项 + 文档同步 4 项; 待修补清单里的 ③面板结论 ④依赖审计真跑(根 lockfile + npm audit 真出结果) ⑤断言 base64 传参 ⑥市场 minApp 强制 ⑦市场发布流程 全部落地。
- **清理**: 删 dist/_旧编号测试包-1.4.1(**700 MB**)与 config.json.bak(含密钥的旧备份, 可再生); 可再生缓存(.electron-cache 138MB / .pydist 86MB / .ocr-langs 40MB)按"删了要重建"保留并记录; 10 份审计报告留作留痕。
- **文档**: 使用说明.txt 新增 B站插件**详细用法**(第九节: 准备/授权/设置一览/行为优先级/直播间信息/FAQ 7 条/卸载); DEV-NOTES 215 **开发数据记录**(38 提交 / +5313−788 行 / 插件 1278 行 / 319 条断言 / 门禁新增 59 条); PROCESS-02 §8.11 发布记录; 卡片 M-20260925-05/06/07 转 FIXED。
- **新功能立项**: 功能卡 F-20260925-02「VR 虚拟键盘」DRAFT + 详细技术研究(SteamVR 覆盖层覆盖范围 / 系统输入法候选窗三条路线 / 语音输入三方案 / 交付形态取舍)+ 3.5 节"复用 LiveTranslate 的可行性"; 功能卡末尾写了**下次开工指引**(两件待用户确认 + 两个第一步动作)。
- **待办(下次)**: ① 用户确认 LiveTranslate 音频输入能否选麦克风; ② 用户拍板交付形态(独立工具 / 随包); ③ P0-A 语音输入模式(纯插件); ④ P0-B VR 打字页; ⑤ 历史遗留 M-20260925-08(4 个官方插件设置迁到 manifest.settings)。
- **状态**: 工作区与 origin 一致, 门禁 14 PASS, 无未完成后台任务; 遗留的"诚实事項"只有一条 —— 桌面令牌文件已删但**建议用户把该 GitHub 令牌轮换一次**。
