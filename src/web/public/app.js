...
if($('healthCopy'))$('healthCopy').onclick=async function(){try{var j=await (await fetch('/api/ports/check')).json();await navigator.clipboard.writeText(JSON.stringify(j,null,2));note(tr('healthCopied'),'ok');}catch(e){apiFail('#healthCopy',e);}};
