...
function plgSettingsForm(p,body){
  ...
  function applyNow(){if((p.apiMethods||[]).indexOf('reloadConfig')<0)return Promise.resolve();return plugCall(p.id,'reloadConfig',{}).catch(function(){});}
  bSave.onclick=function(){msg.style.color='';msg.textContent=tr('plgSaving');save().then(function(j){if(j&&j.ok){markSaved();return applyNow().then(function(){msg.textContent=tr('plgSetSaved');});}msg.textContent=tr('failed')+': '+((j&&j.error)||'');}).catch(function(e){msg.textContent=tr('failed')+': '+e.message;});};
