...
function plgWarn(p,onOk){var m=$('plgModal');if(!m){onOk();return;}m.style.display='flex';$('plgModalTitle').textContent=t('plgModalTitle');$('plgWarnText').innerHTML='<b>'+esc(plgName(p))+'</b> v'+esc(p.version||'')+(p.author?(' · '+esc(p.author)):'')+'<br><span style="color:var(--muted)">'+esc(p.description||'')+'</span><br><br><b>'+t('plgReqPerms')+':</b><br>'+plgPermsHtml(p)+'<br><br><b>'+t('plgNote')+':</b> '+t('plgWarnNote');$('plgCancel').textContent=t('cancel');$('plgCancel').className='gray';var n=5;var btn=$('plgConfirm');btn.disabled=true;btn.textContent=t('plgConfirmCountdown').replace('{n}',n);if(window._plgIv)clearInterval(window._plgIv);window._plgIv=setInterval(function(){n--;if(n<=0){clearInterval(window._plgIv);btn.disabled=false;btn.textContent=t('plgConfirmText');}else{btn.textContent=t('plgConfirmCountdown').replace('{n}',n);}},1000);btn.onclick=function(){if(btn.disabled)return;m.style.display='none';onOk();};$('plgCancel').onclick=function(){m.style.display='none';clearInterval(window._plgIv);};}
...
window.__plgset_weather_board=function(p,body){
  ...
  body.querySelector('[data-wxpreset]').onclick=function(){plugCall('weather-board','addPresets',{kind:'cn'}).then(function(j){msg.textContent=j&&j.ok?(t('wxAddedCn').replace('{n}',(j.added||0))):'失败';plugCall('weather-board','getRows',{}).then(function(j2){rows=Array.isArray(j2)?j2:[];render();});});};body.querySelector('[data-wxpresetw]').onclick=function(){plugCall('weather-board','addPresets',{kind:'world'}).then(function(j){msg.textContent=j&&j.ok?(t('wxAddedWorld').replace('{n}',(j.added||0))):'失败';plugCall('weather-board','getRows',{}).then(function(j2){rows=Array.isArray(j2)?j2:[];render();});});};
...
window.__plgset_netease_lyrics=function(p,body){
  body.innerHTML='<div class="sub" style="margin:0 0 6px">'+t('nlTitle')+'</div>'
    ...
    +'<div class="row" style="gap:8px;flex-wrap:wrap;margin:6px 0"><label class="sub"><input type="checkbox" class="nlTrans" checked> '+t('nlTrans')+'</label><label class="sub" title="其它播放器/媒体在播时, 也通过网易云查找并显示该歌曲的歌词"><input type="checkbox" class="nlOther"> '+t('nlOther')+'</label><label class="sub"><input type="checkbox" class="nlRhythm"> '+t('nlRhythm')+'</label><label class="sub"><input type="checkbox" class="nlTitle" checked> '+t('nlTitleTop')+'</label></div>'
  ...
  body.querySelector('[data-nlsave]').onclick=function(){plugCall('netease-lyrics','saveConfig',collect()).then(function(j){msg.textContent=j&&j.ok?'已保存 ✓':t('failed')+': '+(j&&j.error||'');});};
