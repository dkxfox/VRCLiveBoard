...
function capLoad(){var img=$('capImg'),rect=$('capRect'),sel=$('capSel');if(!img)return;if(rect)rect.style.display='none';if(sel)sel.textContent='';img.onload=function(){capRatio=img.naturalWidth/img.getBoundingClientRect().width;if(sel)sel.textContent='('+img.naturalWidth+'×'+img.naturalHeight+'px)';};img.onerror=function(){if(sel)sel.textContent=t('loadFail');};img.src='/api/capture/preview?t='+Date.now();}
...
if($('capSave'))$('capSave').onclick=async function(){var el=$('capRect'),sel=$('capSel');if(!el||!el.style.width||el.style.display==='none'){if(sel)sel.textContent=t('capHint');return;}var x=Math.round(parseFloat(el.style.left)*capRatio),y=Math.round(parseFloat(el.style.top)*capRatio),w=Math.round(parseFloat(el.style.width)*capRatio),h=Math.round(parseFloat(el.style.height)*capRatio);try{var j=await (await fetch('/api/capture/set',{method:'POST',body:JSON.stringify({mode:'region',region:{x:x,y:y,w:w,h:h}})})).json();if(j&&j.ok){var s=$('transRegion');if(s)s.selectedIndex=1;if(sel)sel.textContent=t('capSaved');setTimeout(function(){var ov=$('capOverlay');if(ov)ov.style.display='none';},700);}}catch(e){}};
...
if($('plgImport'))$('plgImport').onclick=async function(){var f=$('plgZip'),m=$('plgMsg');var path=f?(f.value||'').trim():'';if(!path){alert(t('importNeedPath'));return;}try{var j=await (await fetch('/api/plugins/import',{method:'POST',body:JSON.stringify({path:path})})).json();if(m)m.textContent=(j&&j.ok)?(t('importOk')+(j.id||path)+t('importOk2')):(t('importFail')+((j&&j.error)||''));if(j&&j.ok){if(f)f.value='';setTimeout(loadPlugins,800);}}catch(e){if(m)m.textContent=t('importFail')+e.message;}};
...
(function(){fetch('/api/version').then(function(r){return r.json();}).then(function(j){var v=$('ver');if(v)v.textContent=t('verLine')+(j.version||'')+t('codeName');}).catch(function(){});
  fetch('/api/version/check').then(function(r){return r.json();}).then(function(j){var uh=$('updateHint');if(!uh||!j||!j.newer)return;var url=(j.remote&&j.remote.releaseUrl)||'#';var ver=(j.remote&&j.remote.version)||'';uh.innerHTML='<a href="'+url+'" target="_blank" rel="noopener" style="color:var(--accent)">'+t('updateNew').replace('{ver}',ver).replace('{name}','')+'</a>';}).catch(function(){});})();
if($('healthCopy'))$('healthCopy').onclick=async function(){try{var j=await (await fetch('/api/ports/check')).json();await navigator.clipboard.writeText(JSON.stringify(j,null,2));alert(t('healthCopied'));}catch(e){}};
if($('diagCopy'))$('diagCopy').onclick=async function(){var out=$('diagOut'),txt=out?out.textContent:'';try{await navigator.clipboard.writeText(txt);alert(t('healthCopied'));}catch(e){}};
...
if($('diagBtn'))$('diagBtn').onclick=async function(){var out=$('diagOut'),cp=$('diagCopy');if(out){out.style.display='block';out.textContent='…';}try{var j=await (await fetch('/api/diagnose')).json();if(out)out.textContent=JSON.stringify(j,null,2);if(cp)cp.style.display='inline-block';}catch(e){if(out)out.textContent=t('diagFail')+e.message;}};
